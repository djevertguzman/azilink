...
public class TcpDriverImpl implements TcpDriver {
        ...
        public void onTimer() {
            ...
            if (mRetries >= mMaxRetries) {
                ...
                if (VpnNatEngine.sLog)
                    Log.v("AziLink", "onTimerRetransmit is at retry count " + mRetries);
        ...
        try {
            if (!mBindComplete) {
                ...
                if (!pkt.isConnectRequest()) {
                    if (VpnNatEngine.sLog)
                        Log.v("AziLink", "Tcp::newPacket received non-SYN while in bind mode");
            ...
            if (seq < 0 || seq > mInBuffer.limit()) {
                ...
                if (VpnNatEngine.sLog)
                    Log.v("AziLink", "Tcp::packet Seq out of bounds - saw " + seq + " with limit " + mInBuffer.limit());
                TcpPacket tp = makePacket();
                tp.complete();
                mHost.write(tp);
                return;
            ...
            if (len > 0) {
                ...
                if (VpnNatEngine.sLog)
                    Log.v("AziLink", "Tcp::packet imports " + len + " bytes of data from host");
                newData = true;
            ...
            if (pkt.isAck()) {
                ...
                if (ack > 0 && ack <= mOutBuffer.position()) {
                    ...
                    if (VpnNatEngine.sLog)
                        Log.v("AziLink", "Tcp::Packet before seq=" + mOutSeq + ", pos=" + mOutBuffer.position() + ", fin=" + mOutFinSeq + ",ACK=" + ack);
                    ...
                    if (VpnNatEngine.sLog)
                        Log.v("AziLink", "Tcp::Packet after seq=" + mOutSeq + ", pos=" + mOutBuffer.position() + ", fin=" + mOutFinSeq);
                    ...
                    if (VpnNatEngine.sLog)
                        Log.v("AziLink", "Tcp::Packet rejected ack seq=" + mOutSeq + ", pos=" + mOutBuffer.position() + ", fin=" + mOutFinSeq + ",ACK=" + ack);
                }
            } else {
            ...
            if (pkt.isFin()) {
                ...
                if (fseq < mInBuffer.limit()) {
                    if (VpnNatEngine.sLog)
                        Log.v("AziLink", "Tcp::packet recording FIN at relative sequence " + fseq);
                    mInFinSeq = seq + pkt.getDataLength();
                    mInValid.set(mInFinSeq);
                } else {
                    if (VpnNatEngine.sLog)
                        Log.v("AziLink", "Tcp::packet has incoming FIN but no buffer room");
                }
            }
            ...
            if (mInFinSeq == 0) {
                if (VpnNatEngine.sLog)
                    Log.v("AziLink", "Tcp::packet will act on the FIN immediately");
                mInFinSeq = -1;
                mInValid.clear();
                ...
                if (mOutFinSeq == -2) {
                    if (VpnNatEngine.sLog)
                        Log.v("AziLink", "Tcp::packet has no output FIN, so calling onClosed");
                    mCallback.onClosed();
                    return;
                } else {
                    if (VpnNatEngine.sLog)
                        Log.v("AziLink", "Tcp::packet output FIN is at " + mOutFinSeq);
                }
            }
            ...
            if (mOutFinSeq == -1 && mInFinSeq == -1) {
                if (VpnNatEngine.sLog)
                    Log.v("AziLink", "Tcp::Packet detects FIN sequence complete");
                setRetransmitTimer(0);
    ...
    public void xmit() {
        if (mOutBuffer.position() - mOutNextXmit <= 0) {
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Tcp::xmit nothing to send - pos " + mOutBuffer.position() + " xmit - " + mOutNextXmit);
            return;
        }
        ...
        while (data.hasRemaining()) {
            ...
            tp.complete();
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Tcp::xmit seq=" + mInSeq + ", off=" + mOutNextXmit + ", len=" + tp.getDataLength());
            mHost.write(tp);
    ...
    public void read(ByteBuffer data) {
        ...
        if (VpnNatEngine.sLog)
            Log.v("AziLink", "Tcp::read outputs " + bytesWritten + " bytes bitmap=" + mInValid.toString());
        ...
        if (mInFinSeq == 0) {
            // Pseudo-send it
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Tcp::read reached FIN from host, so showing close");
            mInFinSeq = -1;
            mInSeq = (mInSeq + 1) & 0xFFFFFFFF;
            mInValid.clear();
            wantClose = true;
    ...
    public void write(ByteBuffer data) {
        ...
        if (mOutFinSeq != -2) {
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Tcp::write aborted since output buffer is closing");
            return;
        }
