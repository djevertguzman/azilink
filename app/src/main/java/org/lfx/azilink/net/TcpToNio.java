...
public class TcpToNio extends SocketHandler implements TcpDriverCallback, TimerCallback {
    ...
    public void onRead(SelectionKey k) {
        ...
        if (maxLen <= 0) {
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Nio::onRead is returning because max read length is 0");
            return;
        }
        ...
        if (VpnNatEngine.sLog)
            Log.v("AziLink", "Nio::onRead pass to driver - maxLen=" + maxLen + ", bytesRead=" + bytesRead + "inpos=" + mInBuffer.position());
        mInBuffer.flip();
    ...
    public void onWrite(SelectionKey k) {
        ...
        if (mOutBuffer.position() != 0) {
            ...
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Nio::onWrite recursing into onNewDataAvailable");
            onNewDataAvailable();
        } else if (mCloseWhenDoneXmit) {
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Nio::onWrite closing because xmit is finished");
            mState = State.STATE_NONE;
            mTCP.close();
        }
    }
    ...
    public void onClosed() {
        ...
        if (mOutBuffer.position() == 0) {
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Nio::onClosed is closing immediately since no buffer");
            mState = State.STATE_NONE;
            mTCP.close();
        } else {
    ...
    public void onNewDataAvailable() {
        if (mState != State.STATE_CONNECTED) {
            if (mTCP.getReadAvailableSize() != 0) {
                if (VpnNatEngine.sLog)
                    Log.v("AziLink", "onNewData in unconnected state with data -> destroy tcp");
                mTCP.destroy();
            }
            return;
        }
        ...
        if (mOutBuffer.position() != 0) {
            ...
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Nio::onNewData wrote all " + bytesWritten + " bytes");
        }
    }
