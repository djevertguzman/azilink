...
public class UdpDriver extends SocketHandler implements TimerCallback {
    ...
    public UdpDriver(VpnNatEngine engine, UdpKey nk, UdpPacket pkt, boolean icmp) throws IOException {
        ...
        if (VpnNatEngine.sLog)
            Log.v("AziLink", "Connect to foreign host (udp) " + InetAddress.getByAddress(addr).getHostAddress() + ":" + nk.mDestPort);
    ...
    public void onRead(SelectionKey k) {
        if (VpnNatEngine.sLog)
            Log.v("AziLink", "UDP Foreign->Host " + mAddr.mSrcPort + " and " + mAddr.mDestPort);
