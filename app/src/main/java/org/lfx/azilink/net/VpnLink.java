...
public class VpnLink extends SocketHandler {
    ...
    public void onRead(SelectionKey k) throws IOException {
        ...
        if (mInput.remaining() == 0) {
            ...
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "Ridiculously long VPN packet was received; killing the connection");
            mEngine.lostServerLink(this);
            return;
        }
    }
    ...
    public void write(byte[] d, short length) {
        ...
        try {
            if (VpnNatEngine.sLog)
                Log.v("AziLink", "vpnlink::write transmit " + length + " packet");
        ...
        if (VpnNatEngine.sLog)
            Log.v("AziLink", "vpnlink::write has exceeded buffer -- pausing output");
        mPauseOutput = true;
